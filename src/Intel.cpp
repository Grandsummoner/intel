...
struct IntelWidget : ModuleWidget {
	IntelWidget(Intel* module) {
		...
		// I/O row
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(15.4, 40.0)), module, Intel::EXT_L_INPUT));
		addInput(createInputCentered<PJ301MPort>(mm2px(Vec(30.51, 40.0)), module, Intel::EXT_R_INPUT));
		addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(45.61, 40.0)), module, Intel::MASTER_L_OUTPUT));
		addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(60.72, 40.0)), module, Intel::MASTER_R_OUTPUT));

		// TRIG sub-panels: jack + rate-div/depth button pair
		static const float colX[4] = {15.4f, 30.51f, 45.61f, 60.72f};
		...
		for (int i = 0; i < 4; i++) {
			addOutput(createOutputCentered<PJ301MPort>(mm2px(Vec(colX[i], 56.0)), module, trigOut[i]));
			auto* rd = createParamCentered<IntelColorButton>(mm2px(Vec(colX[i] - 2.6, 63.0)), module, rateDivParam[i]);
			rd->mod = module; rd->lightId = rateDivLight[i];
			addParam(rd);
			auto* dp = createParamCentered<IntelColorButton>(mm2px(Vec(colX[i] + 2.6, 63.0)), module, depthParam[i]);
			dp->mod = module; dp->lightId = depthLight[i];
			addParam(dp);
		}

		// FX buttons
		auto* delayBtn = createParamCentered<IntelButton>(mm2px(Vec(15.4, 84.0)), module, Intel::DELAY_PARAM);
		delayBtn->mod = module; delayBtn->lightId = Intel::DELAY_LIGHT;
		addParam(delayBtn);
		auto* reverbBtn = createParamCentered<IntelButton>(mm2px(Vec(30.51, 84.0)), module, Intel::REVERB_PARAM);
		reverbBtn->mod = module; reverbBtn->lightId = Intel::REVERB_LIGHT;
		addParam(reverbBtn);
		auto* shimBtn = createParamCentered<IntelButton>(mm2px(Vec(45.61, 84.0)), module, Intel::SHIMMER_PARAM);
		shimBtn->mod = module; shimBtn->lightId = Intel::SHIMMER_LIGHT;
		addParam(shimBtn);
		auto* syncBtn = createParamCentered<IntelButton>(mm2px(Vec(60.72, 84.0)), module, Intel::SYNC_PARAM);
		syncBtn->mod = module; syncBtn->lightId = Intel::SYNC_LIGHT;
		addParam(syncBtn);

		// FX knobs
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(11.0, 104.0)), module, Intel::MIX_PARAM));
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(25.75, 104.0)), module, Intel::TIME_PARAM));
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(40.5, 104.0)), module, Intel::REGEN_PARAM));
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(55.25, 104.0)), module, Intel::TONE_PARAM));
		addParam(createParamCentered<RoundBlackKnob>(mm2px(Vec(66.62, 104.0)), module, Intel::SPEED_PARAM));

		// Meter (14 LEDs)
		for (int i = 0; i < 14; i++) {
			float x = 6.5f + i * 4.3f;
			addChild(createLightCentered<SmallLight<GreenLight>>(mm2px(Vec(x, 28.0)), module, Intel::METER_LIGHT + i));
		}
	}
};
